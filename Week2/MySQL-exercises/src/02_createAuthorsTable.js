...
const createAuthorsTableQueries = readAndProcessSqlFile(AUTHORS_TABLE_SQL_FILE);

// Show results after creating the tables
const queryShowResults = `SHOW TABLES FROM authors_researchPapers;`;

export const createAuthorsTable = async () => {
  try {
    console.log("\x1b[1mCreating 'authors' table:\x1b[0m");
    await dbConnectAndQuery(createAuthorsTableQueries, queryShowResults);
  } catch (err) {
